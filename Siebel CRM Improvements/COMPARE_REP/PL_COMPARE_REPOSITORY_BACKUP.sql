DECLARE
            ...
            VSTATUS VARCHAR2(1000);
                ...
                CURSOR C_TABLE IS
                    SELECT 
                        TBC.TABLE_NAME NAME, 
                        (SELECT T.ROW_ID 
                            FROM SIEBEL.S_TABLE@DEV2SBL8 T, SIEBEL.S_REPOSITORY@DEV2SBL8 REP 
                            WHERE T.NAME = TBC.TABLE_NAME 
                            AND REP.NAME = 'Siebel Repository'
                            AND REP.ROW_ID = T.REPOSITORY_ID 
                            AND ROWNUM < 2) ROW_ID,
                        (SELECT BC.NAME 
                            FROM SIEBEL.S_BUSCOMP@DEV2SBL8 BC, SIEBEL.S_REPOSITORY@DEV2SBL8 REP 
                            WHERE 
                ...
                CURSOR C_COLUMN (V_TBL_ID VARCHAR2) IS
                    SELECT  T.NAME TABLE_NAME, 
                            ...
                            C.ROW_ID, 
                            (   SELECT F.NAME 
                                    FROM SIEBEL.S_FIELD@DEV2SBL8 F, SIEBEL.S_BUSCOMP@DEV2SBL8 B 
                                    WHERE 
                            ...
                            (SELECT DATA_TYPE FROM ALL_TAB_COLUMNS@DEV2SBL8 WHERE COLUMN_NAME = C.NAME AND TABLE_NAME = T.NAME AND ROWNUM < 2) DATA_TYPE
                    FROM 
                    ...
                    AND T.NAME || '.' || C.NAME NOT IN (SELECT TABELA || '.' || COLUNA FROM TAB_CTRL_COLUMN_EXCEPTION)
            ...
            BEGIN
                ...
                SELECT NVL(MAX(ID),0) INTO VID FROM CTRL_LOG_COMP_REP TUPD 
                WHERE
                FILA = V_FILA
                ...
                IF VID > 0 THEN  
                    ...
                    SELECT TOTAL_PROCESS_TO_EXEC INTO VTOTAL_PROCESS FROM CTRL_LOG_COMP_REP TUPD 
                    WHERE
                    TUPD.ID = VID;
                    DBMS_OUTPUT.PUT_LINE('teste11');
                    FOR R IN C_TABLE LOOP
                
                        UPDATE CTRL_LOG_COMP_REP TUPD
                        SET 
                        ...
                        SELECT MASTER_LOG_COMPARE_ID INTO VMASTER_ID FROM CTRL_LOG_COMP_REP WHERE ID = VID;
                        DBMS_OUTPUT.PUT_LINE('teste13');
                        INSERT INTO CTRL_LOG_COMP_REP_LOG
                            (
                              TABLE_NAME                  ,
                        ...
                        IF (R.TAB_P3 IS NOT NULL) THEN
                            BEGIN
                                ...
                                FROM TAB_CTRL_BATIMENTO_REP WHERE
                            ...
                            DBMS_OUTPUT.PUT_LINE('teste400');
                                ...
                                V_PL :=   'DECLARE ' ||
                                        ...
                                        '           IF (TRIM(REG.' || RC.COL_NAME || ') <> TRIM(V_VALOR_DES)) THEN ' ||  
                                                    ' INSERT INTO BATIMENTO_REPOSITORIO ' ||    
                                                    ' (' ||
                                                        ' TYPE,' || 
                                                        ' PAR_N0,' ||
                                                        ' PAR_N1,' ||
                               ...
                               V_PL :=  V_PL ||         '  ''' ||  R.TYPE || ''',' || 
                                            ...
                                            'EXCEPTION WHEN NO_DATA_FOUND THEN ' ||
                                                    'SELECT COUNT(1) INTO ICONTADOR FROM BATIMENTO_REPOSITORIO ' ||
                                                    'WHERE ' || 
                                ...
                                EXCEPTION WHEN OTHERS THEN
                                    ...
                                    UPDATE CTRL_LOG_COMP_REP_LOG SET
                                        STATUS = 'PROCESSING WITH ERRORS',
                                        SCRIPT_ERROR = V_PL,
                        ...
                        END LOOP;        
                        UPDATE CTRL_LOG_COMP_REP_LOG SET
                        STATUS = DECODE(STATUS, 'PROCESSING', 'SUCCESS', 'PARTIAL SUCCESS')
                        --,SCRIPT_ERROR = V_PL
                            WHERE 
                            CTRL_LOG_COMP_REP_MASTER_ID = VMASTER_ID
                            AND CTRL_LOG_COMP_REP_ID = VID
                    ...
                    END LOOP;
                END IF;
                --SELECT STATUS INTO VSTATUS FROM CTRL_LOG_COMP_REP WHERE ID = VID;
                ...
                SELECT COUNT(1) INTO V_VALIDA_FIM FROM CTRL_LOG_COMP_REP 
                ...
                IF V_VALIDA_FIM = 0 THEN
                    ...
                    UPDATE CTRL_LOG_COMP_REP_MASTER 
                    SET 
                    LAST_UPD = SYSDATE,
            ...
            EXCEPTION WHEN OTHERS THEN    
                ...
                SELECT COUNT(1) INTO V_VALIDA_LOG FROM CTRL_LOG_COMP_REP_LOG 
                WHERE 
                CTRL_LOG_COMP_REP_MASTER_ID = VMASTER_ID
                AND CTRL_LOG_COMP_REP_ID = VID
                AND FILA = V_FILA;
